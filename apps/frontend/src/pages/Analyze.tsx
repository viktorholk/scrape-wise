...
import { addScrapeJob, getAuthToken, changePromtJob } from "@/services";
...
export default function Analyze() {
  ...
  const handleStopJobInternal = async () => {
    setJob((prev) =>
      prev
        ? { ...prev, inProgress: false, error: "Job manually stopped." }
        : null
    );
    setWsMessages((prev) => [
      ...prev,
      {
        type: "error",
        jobId: job?.results?.crawlerJob.id || 0,
        data: { error: "Job stopped by user." },
              ...
              <Card className="h-[450px]">
                <CardHeader className="h-20">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Enter a URL and extraction instructions, and we'll crawl the
                    website and analyze the content.
                  </p>
                </CardHeader>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2 mt-4">
                        <Label htmlFor="crawl-depth-input" className="h-[20px]">
                          Crawl Depth
                        </Label>
                        <Input
                          ...
                          value={crawlDepth}
                          onChange={(e) =>
                            setCrawlDepth(parseInt(e.target.value, 10))
                          }
                          disabled={!!job?.inProgress}
                        />
                      </div>
                      <div className="space-y-2 mt-4">
                        <Label htmlFor="page-limit-input" className="h-[20px]">
                          Page Limit
                        </Label>
                        <Input
                          id="page-limit-input"
                  ...
                  <ScrollArea className="h-full bg-muted rounded-md p-2">
                    <div className="h-full w-full">
                      {wsMessages.map((msg, idx) => (
                        <div
                          key={idx}
                          className="flex flex-row gap-2 items-start py-1.5 border-b border-slate-200 dark:border-slate-700 last:border-b-0"
                        >
                          ...
                          <div className="flex-grow">
                            ...
                            {msg.type === "crawler_job_progress" && (
                              <span className="text-sm text-slate-600 dark:text-slate-400">
                                Crawling: {msg.data?.url}
                              </span>
                            )}

                            {msg.type === "crawler_job_progress_error" && (
                              <span className="text-sm text-red-600 dark:text-red-400">
                                Error crawling {msg.data?.url}:{" "}
                                {msg.data?.error}
                              </span>
                            )}
                            ...
                            {msg.type === "crawler_job_finished" && (
                              <span className="text-sm">
                                Crawler job finished
                              </span>
                            )}

                            {msg.type === "analyser_job_relevance_started" && (
                              <span className="text-sm">
                                Analyser: Determining relevant pages...
                              </span>
                            )}

                            {msg.type === "analyser_job_relevance_finished" && (
                              <span className="text-sm">
                                Analyser: Relevant page determination complete.
                              </span>
                            )}

                            {msg.type === "analyser_job_analysis_started" && (
                              <span className="text-sm">
                                Analyser: Starting data extraction...
                              </span>
                            )}

                            {msg.type === "analyser_job_analysis_finished" && (
                              <span className="text-sm">
                                Analyser: Data extraction complete.
                              </span>
                            )}
                ...
                <Card className="mt-6">
                  ...
                  <CardContent>
                    <div className="p-4 flex flex-col gap-3">
                      ...
                      <Button
                        onClick={async () => {
                          if (!job.results?.analyserJob?.id || !newPrompt)
                            return;
                          ...
                          try {
                            ...
                            setWsMessages((prev) => [
                              ...prev,
                              {
                                ...
                                data: {
                                  message:
                                    "Re-analysis complete with new prompt.",
                                },
                                timestamp: new Date().toLocaleTimeString(),
                              },
                            ]);
                        ...
                        disabled={
                          changingPrompt ||
                          !newPrompt ||
                          newPrompt === job.results.analyserJob.prompt
                        }
                        variant="default"
                        size="sm"
